...
Let’s import data using the `readr` package and use a relative path to
load data from local file. Use clean names function of janitor package
to clean up my dataset.
...
Use view to open separate window with entire dataset. Need to use
`eval = FALSE` for code to knit successfully.

``` r
View(litters_df)
```

skim function from `skimr` package is nice way to get quick summary of
entire dataset.
...
Treat empty rows, NA, 999, 88 all as missing observations(via
collection)
...
Column types — can see different variable types in tibble print-out
option:
